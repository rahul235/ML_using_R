...
how much of the summary metrics for hold-out sample should be saved. final saves the summary for the optimal tuning paramter only.
> returnResamp='final' 'all' or 'none'

how much of the hold-out predictions for each resample should be saved. final saves the predictions for the optimal tuning paramter only.

> savePredictions ='final' 'all' or 'none'
...
```{r}
num_cores <- makeCluster(detectCores())
num_cores
```
```{r}
#objTune = expand.grid(mtry= c(2:5))
```
...
The prediction object, created using prediction () from ROCR, contains the following information:
...
* n.pos: A list in which each element contains the number of positive samples in the given xvalidation run.
...
More at: https://cran.r-project.org/web/packages/ROCR/ROCR.pdf
...
### Random Forest with down sampling - Approach 1
...
```{r}
search_grid <- expand.grid(mtry = c(2:5))
```
